...
if (isLogged()) {
    $user = $_SESSION['logged'];
    $stmt = $royaldb->query("SELECT * FROM user") or die($royaldb->error);
    if ($stmt->num_rows > 0) {
        $Users = $stmt;
    }
}
...
function sendMail($subject, $msg, $email)
{
    ...
    $mail->SMTPAuth = true;
    $mail->SMTPSecure = "tls";
    $mail->Username = "support@qtfx.space";
    $mail->Password = 'urBO2lsy29aa';
    $mail->setFrom('support@qtfx.space', 'QTFx');
    $mail->setFromName = 'QTFx';
    $mail->addReplyTo("support@qtfx.space");
