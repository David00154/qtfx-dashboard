...
$getdeposit = $royaldb->query("SELECT * FROM withdrawal WHERE id > 0 ORDER BY id DESC") or die($royaldb->error);
$getdepo = "";
if ($getdeposit->num_rows > 0) {
    ...
    while ($loaddepo = $getdeposit->fetch_object()) {
        ...
        $getWdate = date("M-d-Y", $loaddepo->date);
        $getdepo .= "<tr><td>$sn</td><td>$user</td><td>$loaddepo->amount BTC</td><td>$getWdate</td><td>$status</td><td><a href='view_withdrawal.php?id=$loaddepo->id' class='btn btn-xs btn-primary'>Approve</a></td></tr>";
        $sn++;
    }
} else {
